...
parser.add_argument('--numneg', help='Number of negative examples, default 15')
...
for epoch in range(NUM_EPOCHS):
    ...
    for pair in pairs:
        ...
        running_avg_objective.append(CUR_OBJECTIVE)
        if(numDone%50000 == 0 and numDone != 0):
            print('Avg objective = ', np.sum(running_avg_objective)/50000)
            running_avg_objective = []
        numDone += 1
    ...
    if(epoch%5 == 0):
        # write to temp file
        suffix = 'NEG_' + str(NUM_NEGATIVE_SAMPLES) + '_DIM_' + str(EMBEDDING_DIMENSION) + '_EPOCHS_' + str(NUM_EPOCHS) + '_' + OUTFILES_SUFFIX + 'temp_epoch_' + str(epoch)
        print('Saving at epoch ', epoch)
        np.savez('data/W_'+suffix +'.npy', W)
        np.savez('data/C_'+suffix +'.npy', C)
...
np.savez('data/W_'+suffix +'.npy', W)
np.savez('data/C_'+suffix +'.npy', C)
# print('Loaded from', 'data/W_NEG_20_DIM_300_EPOCHS_50_nonum_full_win3.txt')
print('Saving done')
